...
(* inflation rate (1.0-5.0%) affects product prices and order volumes. Round to 2 digits. *)
fun increase_inflation(infl: Inflation) = 
let
	val delta = uniform(1.0, 6.0)
	val new_infl = (Real.fromInt (round(100.0*delta)))/100.0 + infl
in
   new_infl
end;

fun adapt_prices_to_inflation([], _, _) = [] | adapt_prices_to_inflation((prod,weight,price)::pis: ProductInfos, old_infl: Inflation, new_infl: Inflation) = 
let 
   val new_price = price*(1.0+new_infl/100.0)/(1.0+old_infl/100.0)
   (* round to 2 digits *)
   val new_rounded_price = (Real.fromInt (round(100.0*new_price)))/100.0
in
   (prod,weight,new_rounded_price)::adapt_prices_to_inflation(pis, old_infl, new_infl)
end;


(* resource getters *)

fun role_filter_employees([], _) = [] | role_filter_employees((name, empl_role)::empls: Employees, role) = if empl_role = role then (name, role)::role_filter_employees(empls, role) else role_filter_employees(empls, role)

fun pick_employee_by_role(empls: Employees, role) = 
let
	val specialists = role_filter_employees(empls, role)
	val n = length specialists
	val rnd_index = round(uniform(0.0, Real.fromInt n) - 0.5)
in
	List.nth(specialists, rnd_index)
end;

fun pick_employee_by_id([], _) = ("INVALID", Sales) | pick_employee_by_id(empl::empls: Employees, id) = 
let
   val empl_id = #1 empl
in
   if empl_id = id then empl else pick_employee_by_id(empls, id)
end;

fun pick_customer_by_id([], _) = ("INVALID", "INVALID", "INVALID") | pick_customer_by_id(cust::custs: Customers, id) = 
let
   val cust_id = #1 cust
in
   if cust_id = id then cust else pick_customer_by_id(custs, id)
end;

fun pick_shipper(empls: Employees) = pick_employee_by_role(empls, Shipment)
fun pick_salesman(empls: Employees) = pick_employee_by_role(empls, Sales)
fun pick_warehouser(empls: Employees) = pick_employee_by_role(empls, Warehousing)


(* simulate understaffing on shop floor (naive) *)
fun pick_forwarder_employee(empls: Employees, (pid, its, cust, weight, shipper): Package) =
let
	val b1 = fbool(0.9)
	val b2 = fbool(0.7)
in
	if b1 then pick_warehouser(empls) else if b2 then shipper else pick_shipper(empls)
end;


(* Sales *)
fun pick_salesperson_for_customer(empls: Employees, custs: Customers, cid) = 
let
   val specialists = role_filter_employees(empls, Sales)
   val customer = pick_customer_by_id(custs, cid)
   val eid1 = #2 customer
   val eid2 = #3 customer
   val b1 = fbool(0.864)
   val b2 = fbool(0.7)
in
   if b1 then pick_employee_by_id(empls, eid1) else if b2 then pick_employee_by_id(empls, eid2) else pick_employee_by_role(empls, Sales)
   (*pick_employee_by_id(empls, eid1)*)
end;